...
//step 2.) Top INSPTCTX_SIGNATURES_TOTAL masternodes, open connect to top 1 masternode.
//         Send "txvote", CTransaction, Signature, Approve
//step 3.) Top 1 masternode, waits for INSPTCTX_SIGNATURES_REQUIRED messages. Upon success, sends "txlock'

void ProcessMessageInstantX(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    if (fLiteMode) return; //disable all Darksend/masternode related functionality
    if (!IsSporkActive(SPORK_2_INSPTCTX)) return;
    ...
    if (strCommand == "ix") {
        ...
        if (fAccepted) {
            ...
            if (i != mapTxLocks.end()) {
                ...
                if ((*i).second.CountSignatures() >= INSPTCTX_SIGNATURES_REQUIRED) {
...
void DoConsensusVote(CTransaction& tx, int64_t nBlockHeight)
{
    ...
    int n = mnodeman.GetMasternodeRank(activeMasternode.vin, nBlockHeight, MIN_INSPTCTX_PROTO_VERSION);
...
{
    int n = mnodeman.GetMasternodeRank(ctx.vinMasternode, ctx.nBlockHeight, MIN_INSPTCTX_PROTO_VERSION);
...
bool CTransactionLock::SignaturesValid()
{
    BOOST_FOREACH (CConsensusVote vote, vecConsensusVotes) {
        int n = mnodeman.GetMasternodeRank(vote.vinMasternode, vote.nBlockHeight, MIN_INSPTCTX_PROTO_VERSION);
