...
#define URI_SCHEME "purpleturtle"
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a PTC address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no PTC: URI
...
{
    // Convert purpleturtle:// to purpleturtle:
    //
    //    Cannot handle this later, because purpleturtle:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open purpleturtle.conf with the associated application */
...
{
    return GetAutostartDir() / "purpleturtle.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a purpleturtle.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the purpleturtle app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add purpleturtle app to startup item list
