...
class TxViewDelegate2 : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate2() : QAbstractItemDelegate(), unit(BitcoinUnits::PTC)
    {
    }
...
void Coinmix::updateDarksendProgress()
{
    ...
    QString strAnonymizePTCAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizePTCAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizePTCAmount = strAnonymizePTCAmount.remove(strAnonymizePTCAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizePTCAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizePTCAmount * COIN) nMaxToAnonymize = nAnonymizePTCAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizePTCAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizePTCAmount));
        strAnonymizePTCAmount = strAnonymizePTCAmount.remove(strAnonymizePTCAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizePTCAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
