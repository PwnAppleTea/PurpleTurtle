...
#if defined(NDEBUG)
#error "PurpleTurtle cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "PurpleTurtle Signed Message:\n";
...
bool IsStandardTx(const CTransaction& tx, string& reason)
{
    ...
    // to MAX_SPTCDARD_TX_SIZE mitigates CPU exhaustion attacks.
    unsigned int sz = tx.GetSerializeSize(SER_NETWORK, CTransaction::CURRENT_VERSION);
    if (sz >= MAX_SPTCDARD_TX_SIZE) {
        reason = "tx-size";
        return false;
    }
...
int GetInputAgeIX(uint256 nTXHash, CTxIn& vin)
{
    ...
    if (nResult < 6) {
        ...
        if (sigs >= INSPTCTX_SIGNATURES_REQUIRED) {
...

    {
        ...
        if (nSigOps > MAX_TX_SIGOPS)
            return state.DoS(0,
                ...
                REJECT_NONSPTCDARD, "bad-txns-too-many-sigops");
        ...
        if (!CheckInputs(tx, state, view, true, SPTCDARD_SCRIPT_VERIFY_FLAGS, true)) {
        ...
        if (!CheckInputs(tx, state, view, true, MANDATORY_SCRIPT_VERIFY_FLAGS, true)) {
            return error("AcceptToMemoryPool: : BUG! PLEASE REPORT THIS! ConnectInputs failed against MANDATORY but not SPTCDARD flags %s", hash.ToString());
        }
...
bool AcceptableInputs(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool isDSTX)
{
	...
	*/
...

    {
        ...
        // Check against previous transactions
        // This is done last to help prevent CPU exhaustion denial-of-service attacks.
        if (!CheckInputs(tx, state, view, false, SPTCDARD_SCRIPT_VERIFY_FLAGS, true)) {
        ...
        //        {
        //            return error("AcceptableInputs: : BUG! PLEASE REPORT THIS! ConnectInputs failed against MANDATORY but not SPTCDARD flags %s", hash.ToString());
        //        }
        ...
        if (fScriptChecks) {
            for (unsigned int i = 0; i < tx.vin.size(); i++) {
                ...
                if (pvChecks) {
                    ...
                    if (flags & SPTCDARD_NOT_MANDATORY_VERIFY_FLAGS) {
                        ...
                        CScriptCheck check(*coins, tx, i,
                            flags & ~SPTCDARD_NOT_MANDATORY_VERIFY_FLAGS, cacheStore);
                        if (check())
                            return state.Invalid(false, REJECT_NONSPTCDARD, strprintf("non-mandatory-script-verify-flag (%s)", ScriptErrorString(check.GetScriptError())));
                    }
...
{
    RenameThread("purpleturtle-scriptch");
...
*/
    ...
    if (IsSporkActive(SPORK_3_INSPTCTX_BLOCK_FILTERING)) {
        BOOST_FOREACH (const CTransaction& tx, block.vtx) {
            if (!tx.IsCoinBase()) {
