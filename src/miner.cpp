...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("PTCMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("PTCMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("PTCMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("purpleturtle-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("PTCMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running PTCMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadPTCMiner() exception");
    } catch (...) {
        LogPrintf("ThreadPTCMiner() exception");
    }

    LogPrintf("ThreadPTCMiner exiting\n");
}
