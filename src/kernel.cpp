...
{
    //PTC will hash in the transaction hash and the index number in order to make sure each hash is unique
...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake)
{
    ...
    if (!VerifyScript(txin.scriptSig, txPrev.vout[txin.prevout.n].scriptPubKey, SPTCDARD_SCRIPT_VERIFY_FLAGS, TransactionSignatureChecker(&tx, 0)))
